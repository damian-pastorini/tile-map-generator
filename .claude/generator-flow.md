...
If none of the three sources yields tiles, every `TilesShortcuts` slot (`sTC`, `cTL`, `cTR`, …) is `undefined`. That state used to be written straight into the layers, because:

- `WallsGenerator.isTopBorderTile(undefined, shortcuts)` matched, since `undefined === shortcuts.cTL` is true when both are undefined
- `determineWallTiles()` then returned `[undefined, undefined]`, which is a truthy array, so `placeWallTiles()` stamped `undefined` tiles into the walls layer

`isTopBorderTile()` now returns false for any falsy tile, and `fetchResolvedWallTiles()` returns `null` (with a debug log) when either wall tile is unresolved. A spot with no resolvable tiles now produces empty layers instead of layers full of `undefined`. Note `TileCountingUtility.countNonZeroTiles()` counts `undefined` as a tile, so any test asserting only "non zero tiles exist" can pass on that corrupted output: assert against the expected tile IDs instead.

### Outer walls no longer require inner walls

`WallsGenerator.placeOuterWallTile()` skips a position when `wallsLayer[tileIdx]` is already taken. When `borderInnerWalls` is false, `wallsLayer` is `false`, so `false[tileIdx]` is `undefined` and `0 !== undefined` was always true, so every outer wall tile was skipped, meaning outer walls silently required inner walls. The check is now `wallsLayer && 0 !== wallsLayer[tileIdx]`.
...
`spotTilesShortcuts` slot is undefined, and `isTopBorderTile()` can never match, so the inner walls layer
comes out empty even when `borderInnerWalls` is true.

### `applyCornersTiles` is the master switch for the spot ring

`TilesetCompositeConfigBuilder.buildGroundSpotConfig()` derives `applyCornersTiles` and emits it into the
`groundSpots` config. When it is false:
- `SpotBordersAndCorners.applySplitBordersAndCorners()` returns the main layer untouched, so no border tiles
  exist and the emitted `-borders` layer is only a copy of the spot layer
- the whole borders and walls block in `SpotGenerator.generateSpots()` is skipped, so no inner or outer walls
  layer is created at all

It is true when the spot has surrounding tiles, or corner tiles, or `borderInnerWalls`, or `borderOuterWalls`.
...
The hand written dungeon example sets them to `-collisions` and `-collisions-over-player`.
...
`PathConnector` passes the path borders layer into `PathTilesFinisher`, which calls the same
`WallsGenerator.createLayerInnerWalls()` / `createLayerOuterWalls()` used by spots. The emitted layers are
`'path-borders-inner-walls'` and `'path-borders-outer-walls'`, each with `splitBordersLayerSuffix` appended.
The tiles key names the wangset the wall tiles are read from, so `pathsInnerWallsTilesKey: 'cave'` resolves
the `'cave-inner-walls'` wangset.

The map border drawn by `blockMapBorder` has no wall mechanism of its own: `MapBorderGenerator` writes a one
tile ring into `borderLayer` and the only consumer afterwards is `MapLayersComposer`, which emits it as
`collisions-map-border`. Walls on a room therefore come from a ground spot or from the paths, never from the
map border.
...
`MapBorderGenerator.createEntryPosition()` opens a gap in the map border and emits a
`return-to-main-map-change-points` layer. It does nothing unless `entryPosition` is a non empty string AND
`entryPositionSize` is greater than zero, both of which default to empty and 0.
