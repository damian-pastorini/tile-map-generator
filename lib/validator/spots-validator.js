...
class SpotsValidator extends MapValidator
{
    ...
    findSpotInMap(map, spotKey)
    {
        ...
        for(let layer of layers){
            ...
            let nonZeroPositions = this.elementPositionAnalyzer.findTilePositions(layerData, width, height, tile => 0 !== tile);
    ...
    findNearestPathPosition(spotCenter, pathPositions)
    {
        ...
        let minDistance = this.geometryCalculator.calculateDistanceBetweenPositions(spotCenter, nearestPath);
        for(let i = 1; i < pathPositions.length; i++){
            let distance = this.geometryCalculator.calculateDistanceBetweenPositions(spotCenter, pathPositions[i]);
