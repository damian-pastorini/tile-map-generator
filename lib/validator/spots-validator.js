...
class SpotsValidator extends MapValidator
{
    ...
    validateSpotQuantities(map, config)
    {
        ...
        for(let spotKey of spotKeys){
            ...
            validation.violations.push({
                ...
                expected: 1,
                found: spotsFound,
                issue: 0 === spotsFound ? 'missing-spot' : 'duplicate-spots'
            });
            validation.isValid = false;
        }
        validation.accuracyPercentage = 0 === validation.totalExpected ? 100 : (validation.correctQuantities / validation.totalExpected) * 100;
        return validation;
    }
    ...
    validateSpotConnectivity(map, config)
    {
        let groundSpots = sc.get(config, 'groundSpots', {});
        ...
        for(let spotKey of spotKeys){
            let connectivityValidation = this.validateSingleSpotConnectivity(map, spotKey, pathTile, pathLayer);
        ...
        validation.connectivityPercentage = 0 === validation.totalSpots ? 100 : (validation.connectedSpots / validation.totalSpots) * 100;
        return validation;
    }

    validateSingleSpotConnectivity(map, spotKey, pathTile, pathLayer)
    {
