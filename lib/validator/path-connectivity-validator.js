...
class PathConnectivityValidator extends MapValidator
{
    ...
    findAllElementPositions(map, config)
    {
        ...
        for(let elementType of Object.keys(elementsQuantity)){
            let positions = this.elementPositionAnalyzer.findElementPositionsInMap(map, elementType);
    ...
    findNearestPathPosition(elementPosition, map, pathTile)
    {
        ...
        let minDistance = this.geometryCalculator.calculateDistanceBetweenPositions(elementPosition, nearestPath);
        for(let i = 1; i < pathPositions.length; i++){
            let distance = this.geometryCalculator.calculateDistanceBetweenPositions(elementPosition, pathPositions[i]);
    ...
    measureHorizontalPathWidth(centerPos, layerData, width, height, pathTile)
    {
        ...
        for(let dx = 1; dx < width; dx++){
            ...
            if(leftX < 0 || layerData[leftIndex] !== pathTile){
                break;
            }
        }
        for(let dx = 1; dx < width; dx++){
    ...
    measureVerticalPathWidth(centerPos, layerData, width, height, pathTile)
    {
        ...
        for(let dy = 1; dy < height; dy++){
            ...
            if(upY < 0 || layerData[upIndex] !== pathTile){
                break;
            }
        }
        for(let dy = 1; dy < height; dy++){
