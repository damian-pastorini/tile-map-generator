    ...
    validateElementQuantities(map, config, generator = null)
    {
        ...
        for(let elementType of elementTypes){
            ...
            if(!isMatch){
                ...
                this.logValidationError('Element quantity mismatch for '+elementType+': expected '+expectedCount+', actual '+actualCount);
            }
        }
        ...
        for(let y = 0; y <= mapHeight - elementHeight; y++){
            for(let x = 0; x <= mapWidth - elementWidth; x++){
                ...
                for(let elementLayer of elementLayers){
                    ...
                    if(!this.doesPatternMatch(mapLayer.data, mapWidth, elementLayer.data, elementWidth, elementHeight, x, y)){
                        allMatch = false;
                        break;
                    }
                    matchedLayers++;
