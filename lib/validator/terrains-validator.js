...
class TerrainsValidator extends MapValidator
{
    ...
    matchesTerrainName(terrainName, terrainNamePart)
    {
        if('' === terrainNamePart){
            return -1 === terrainName.indexOf('-walls');
        }
        return terrainName.endsWith(terrainNamePart);
    }

    fetchTerrainScopedLayers(map, layerNamePart)
    {
        let terrain = this.fetchTerrainByNamePart(map, layerNamePart);
        let terrainKey = terrain ? terrain.name.substring(0, terrain.name.length - layerNamePart.length) : '';
        if('' === terrainKey){
            return LayerUtility.findLayers(map, layerNamePart);
        }
        return LayerUtility.findLayers(
            map,
            layerNamePart,
            'contains',
            (layerName) => -1 !== layerName.indexOf(terrainKey)
        );
    }

    fetchTerrainByNamePart(map, terrainNamePart)
    {
        let tileset = this.fetchMapTileset(map);
        if(!tileset){
            return null;
        }
        for(let terrain of sc.get(tileset, 'wangsets', [])){
            if(this.matchesTerrainName(sc.get(terrain, 'name', ''), terrainNamePart)){
                return terrain;
            }
        }
        return null;
    ...
    fetchTerrainShortcuts(map, terrainNamePart)
    {
        let terrain = this.fetchTerrainByNamePart(map, terrainNamePart);
        if(!terrain){
            return null;
        }
        let terrainPositions = this.mapTerrainPositions(
            terrain.name,
            terrain.wangtiles,
            sc.get(this.fetchMapTileset(map), 'firstgid', 1)
        );
        return new TilesShortcuts(
            0,
            terrainPositions.surroundingTilesPosition,
            terrainPositions.cornersPosition,
            '',
            terrainPositions
        );
    }
