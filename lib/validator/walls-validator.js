...
class WallsValidator extends MapValidator
{
    ...
    {
        let applyPathsInnerWalls = sc.get(config, 'applyPathsInnerWalls', false);
        if(!applyPathsInnerWalls){
            return {isValid: true, reason: 'Inner walls not configured'};
        }
        let pathTile = sc.get(config, 'pathTile', 0);
        let innerWallsLayer = this.findLayerByName(map, 'path-borders-inner-walls');
        if(!innerWallsLayer){
            return {isValid: false, reason: 'Inner walls layer not found'};
        }
        let pathLayer = this.findLayerByName(map, 'path');
        if(!pathLayer){
            return {isValid: false, reason: 'Path layer not found for inner walls validation'};
        }
        ...
        let validation = {
            isValid: true,
            totalPathTiles: pathPositions.length,
            correctInnerWalls: 0,
            incorrectInnerWalls: 0,
            missingInnerWalls: 0,
            violations: []
        };
        for(let pathPos of pathPositions){
            let innerWallValidation = this.validateInnerWallsAroundPath(pathPos, innerWallsLayer.data, width, height, config);
        ...
        validation.accuracyPercentage = 0 === validation.totalPathTiles ? 100 : (validation.correctInnerWalls / (validation.correctInnerWalls + validation.incorrectInnerWalls + validation.missingInnerWalls)) * 100;
        return validation;
    }

    validateInnerWallsAroundPath(pathPos, innerWallsData, width, height, config)
    {
        let surroundingTiles = sc.get(config, 'surroundingTiles', {});
        let neighbors = this.getPathNeighbors(pathPos.x, pathPos.y, width, height, innerWallsData);
        let validation = {
            correctWalls: 0,
            incorrectWalls: 0,
            missingWalls: 0,
    ...
    {
        let applyPathsOuterWalls = sc.get(config, 'applyPathsOuterWalls', false);
        if(!applyPathsOuterWalls){
            return {isValid: true, reason: 'Outer walls not configured'};
        }
        let outerWallsLayer = this.findLayerByName(map, 'path-borders-outer-walls');
        if(!outerWallsLayer){
        ...
        let width = sc.get(map, 'width', 0);
        let height = sc.get(map, 'height', 0);
        let innerWallPositions = this.findTilePositions(innerWallsLayer.data, width, height, tile => 0 !== tile);
        let validation = {
            isValid: true,
        ...
        for(let innerWallPos of innerWallPositions){
            let outerWallValidation = this.validateOuterWallsAroundInnerWall(innerWallPos, outerWallsLayer.data, width, height, config);
    ...
    validateOuterWallsAroundInnerWall(innerWallPos, outerWallsData, width, height, config)
    {
        ...
        let surroundingTiles = sc.get(config, 'surroundingTiles', {});
        let expectedOuterWalls = Object.keys(surroundingTiles);
        for(let wallKey of expectedOuterWalls){
            let expectedTile = surroundingTiles[wallKey];
            let actualTile = sc.get(neighbors, wallKey, null);
            if(null === actualTile){
                continue;
            }
            if(0 === actualTile){
    ...
    validateCornerTilePlacement(map, config)
    {
        ...
        for(let tile of cornerTileValues){
            let positions = this.findTilePositions(innerWallsLayer.data, width, height, tile);
            cornerPositions.push(...positions);
        }
        for(let cornerPos of cornerPositions){
            let cornerValidation = this.validateSingleCornerPlacement(cornerPos, innerWallsLayer.data, width, height, corners);
    ...
    validateSingleCornerPlacement(cornerPos, layerData, width, height, corners)
    {
        ...
        for(let cornerDef of cornerPositions){
            ...
            if(actualTile === expectedTile){
                let neighbors = this.getPathNeighbors(cornerPos.x, cornerPos.y, width, height, layerData);
                let hasCorrectNeighborPattern = this.validateCornerNeighborPattern(neighbors, cornerDef.key);
                if(hasCorrectNeighborPattern){
    ...
    validateCornerNeighborPattern(neighbors, cornerKey)
    {
        let cornerPatterns = {
            '-1,-1': ['0,-1', '-1,0'],
            '-1,1': ['0,1', '-1,0'],
        ...
        let expectedPattern = sc.get(cornerPatterns, cornerKey, []);
        for(let patternKey of expectedPattern){
            let neighborTile = sc.get(neighbors, patternKey, 0);
            if(0 === neighborTile){
                return false;
            }
        }
