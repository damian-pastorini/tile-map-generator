...
class WallsValidator extends MapValidator
{
    ...
    validateInnerWallPlacement(map, config)
    {
        ...
        let borderPositions = this.findTilePositions(spotBordersLayer.data, width, height, tile => 0 !== tile);
        let validation = {
            isValid: true,
        ...
        for(let borderPos of borderPositions){
            let innerWallValidation = this.validateInnerWallsAroundBorder(borderPos, innerWallsLayer.data, width, height, surroundingTiles);
    ...
    validateOuterWallPlacement(map, config)
    {
        ...
        for(let innerWallPos of innerWallPositions){
            let outerWallValidation = this.validateOuterWallsAroundInnerWall(innerWallPos, outerWallsLayer.data, width, height, surroundingTiles);
