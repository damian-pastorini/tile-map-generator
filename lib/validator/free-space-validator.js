...
class FreeSpaceValidator extends MapValidator
{
    ...
    validateFreeSpaceMinimums(map, config)
    {
        ...
        for(let i = 0; i < elementPositions.length; i++){
            for(let j = i + 1; j < elementPositions.length; j++){
                ...
                let requiredDistance1 = sc.get(elementsFreeSpaceAround, element1.elementType, minimumElementsFreeSpaceAround);
                let requiredDistance2 = sc.get(elementsFreeSpaceAround, element2.elementType, minimumElementsFreeSpaceAround);
        ...
        validation.compliancePercentage = 0 === (validation.validDistances + validation.violatedDistances) ? 100 : (validation.validDistances / (validation.validDistances + validation.violatedDistances)) * 100;
        return validation;
    }
    ...
    validateSingleElementBoundary(element, freeSpaceRequired, map, config)
    {
        ...
        for(let dy = -freeSpaceRequired; dy <= freeSpaceRequired; dy++){
            for(let dx = -freeSpaceRequired; dx <= freeSpaceRequired; dx++){
                ...
                let isElementTile = checkX >= element.x && checkX < element.x + element.width && checkY >= element.y && checkY < element.y + element.height;
                if(isElementTile){
                    continue;
                }
    ...
    getAllowedTilesInFreeSpace(element, config)
    {
        ...
        let allowPaths = sc.get(elementsAllowPathsInFreeSpace, element.elementType, defaultElementsAllowPathsInFreeSpace);
        if(allowPaths){
