...
class RandomMapGenerator
{
    ...
    setOptions(options)
    {
        ...
        this.elementsFreeSpaceAround = sc.get(options, 'elementsFreeSpaceAround', null);
        ...
        this.totalStaticLayers = 1
            ...
            + Object.keys(this.groundSpots).length
    ...
    generateLayersList()
    {
        ...
        let nextLayerId = 4;
        for(let i of Object.keys(this.groundSpots)){
            nextLayerId++;
            let groundSpotConfig = this.groundSpots[i];
            let layerName = groundSpotConfig.layerName || 'ground-spot-'+i;
            this.staticLayers.push(
                this.generateLayerWithData(
                    layerName,
                    this.generateGroundSpotLayerData(i, groundSpotConfig, layerName),
                    nextLayerId
                )
            );
        }
        ...
        for(let y = 0; y < elementData.height; y++){
            for(let x = 0; x < elementData.width; x++){
                ...
                if((0 === y || (elementData.height - 1) === y) && (0 === x || (elementData.width - 1) === x)){
                    this.markFreeSpaceAroundElementAsNotAvailable(freeSpaceAround, gridY, gridX);
                }
    ...
    markFreeSpaceAroundElementAsNotAvailable(freeSpaceAround, gridY, gridX)
    {
        ...
        for(let s = 1; s <= freeSpaceAround; s += freeSpaceAround) {
            ...
            if(-1 === this.temporalBlockedPositionsToAvoidElements.indexOf(savaPoints)){
                this.temporalBlockedPositionsToAvoidElements.push({previousTileY, previousTileX, nextTileY, nextTileX});
            }
        }
    }
    ...
    {
        Logger.debug('Unblock temporal blocked points.', temporalBlockedPositionsToAvoidElements);
        for(let positionPoints of temporalBlockedPositionsToAvoidElements){
    ...
    generateGroundSpotLayerData(spotKey, layerName, groundSpotConfig)
    {
        // @TODO - BETA - Finish implementation.
        /*
        // groundSpotConfig samples:
        respawnPunchTrees: {
        ...
        */
        let spotTile = groundSpotConfig.spotTile || this.elementsProvider.groundSpots[spotKey];
        let layer = Array(this.mapWidth * this.mapHeight).fill(0);
        let spotsQuantity = Number(groundSpotConfig.quantity || 0);
        // map data:
        // let layer = this.additionalLayers[layerIndex]; // layers saved in additionalLayers
        // layer.data[mapIndex] = elementData.data[tileIndex]; // this change the tile on the map, from 0 to tileIndex
        // this.mapGrid[gridY][gridX] = false; // this is used to make tiles walkable or not
        if(0 < spotsQuantity){
            for(let i = 0; i < spotsQuantity; i++){
                if(groundSpotConfig.blockSpace){
                    // the spot place can't be used for anything else
                }
                if(!groundSpotConfig.walkable){
                    // block ground on normal map grid
                }
                if('use' === groundSpotConfig.mapSpace){
                    // check for walkable spaces on the current map on which the groundSpot could be placed
                    continue;
                }
                if('add' === groundSpotConfig.mapSpace){
                    // add spot as element for the map size
                    continue;
                }
