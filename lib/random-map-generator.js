...
class RandomMapGenerator
{
    ...
    applyPatternsThroughMerge(outerWallsLayer, bordersLayer, mapWidth, mapHeight, spotTiles, outerTiles)
    {
        ...
        this.replaceSequence(
            mergedLayers,
            [sBC, cTR, outerTiles.sML, outerTiles.sBL, outerTiles.sBR],
            [sBC, sTL, cTL, outerTiles.sMR, outerTiles.sBR],
            mapWidth
        );
        this.rollbackRotation90Degrees(mergedLayers, mapWidth, mapHeight);
        // and rollback.
        this.replaceSequence(mergedLayers, [cTR, cTL, sTL, cTL], [cTR, sBC, sBC, cTL], mapWidth);
        this.replaceSequence(
            mergedLayers,
        ...
        this.replaceSequence(
            mergedLayers,
            [outerTiles.sBR, outerTiles.sBC, outerTiles.sBL, outerTiles.sBC],
            [outerTiles.sBR, outerTiles.sBC, outerTiles.sBC, outerTiles.sBC],
            mapWidth
        );
        ...
        this.replaceSequence(
            mergedLayers,
            [outerTiles.sBC, outerTiles.sBR, outerTiles.sBL],
            [outerTiles.sBC, outerTiles.sBC, outerTiles.sBL],
            mapWidth
        );
        if(outerTc){
        ...
        for(let i = 0; i < mergedLayers.length; i++){
            ...
            if(cBR === outerWallsLayer[i] && (outerTiles.sMC === outerWallsLayer[i - 1] || outerTiles.sML === outerWallsLayer[i - 1])){
                bordersLayer[i] = outerTiles.sMC;
            }
            if(cBL === outerWallsLayer[i] && (outerTiles.sMC === outerWallsLayer[i + 1] || outerTiles.sMR === outerWallsLayer[i + 1])){
                bordersLayer[i] = outerTiles.sMC;
            }
        }
    ...
    replaceCornersPatters(fixedLayer, mapWidth, spotTiles, outerTiles)
    {
        this.replaceSequences(fixedLayer, [
            ...
            [[0, outerTiles.sBL, 0], [0, 0, 0]],
            [[0, outerTiles.sBR, 0], [0, 0, 0]]
        ], mapWidth);
    }
