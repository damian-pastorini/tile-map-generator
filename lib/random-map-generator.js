...
class RandomMapGenerator
{
    ...
    connectPaths()
    {
        ...
        this.splitBorderLayer = this.applyCleanPathBorderTiles();
    ...
    async generateSpots()
    {
        for(let spotKey of Object.keys(this.groundSpots)){
            ...
            for(let i = 0; i < spotsQuantity; i++){
                ...
                let additionalBordersLayer = this.applySplitBordersAndCorners(
                    groundSpotConfig.applyCornersTiles,
                    groundSpotConfig.width,
                    groundSpotConfig.height,
                ...
                if(groundSpotConfig.applyCornersTiles){
                    ...
                    if(groundSpotConfig.borderInnerWalls){
                        wallsLayer = this.createSpotInnerWalls(additionalBordersLayer, tilesKey, spotTilesShortcuts);
                    }
                    if(groundSpotConfig.borderOuterWalls){
    ...
    async createSpotOuterWalls(additionalBordersLayer, tilesKey, spotTiles, wallsLayer)
    {
        let outerWallsTiles = this.fetchTilesShortcutsBySuffix(tilesKey, spotTiles, '-outer-walls');
        let width = Math.sqrt(additionalBordersLayer.length);
        let height = additionalBordersLayer.length / width;
        let outerWallsLayerData = Array(additionalBordersLayer.length).fill(0);
        let mapper = new WallsMapper(spotTiles, outerWallsTiles);
        // this will be added to the tile index to place the tiles in the outerWallsLayerData:
        let mappedPositions = mapper.mappedPositions();
        let oppositeTiles = mapper.oppositeTiles();
        for(let y = 0; y < height; y++){
            for(let x = 0; x < width; x++){
                let borderTileIndex = y * width + x;
                let borderTile = additionalBordersLayer[borderTileIndex];
                if(0 === borderTile){
                    continue;
                }
                ...
                outerWallsLayerData = await this.placeOuterWallTile(
                    outerWallsLayerData,
                    x,
                    y,
                    width,
        ...
        return this.applyOuterWallsPatterns(outerWallsLayerData, width, height, spotTiles, outerWallsTiles);
    }

    applyOuterWallsPatterns(outerWallsLayerData, mapWidth, mapHeight, spotTiles, outerTiles)
    {
        // @TODO - BETA - Extract into an OuterWallsPatterns class.
        let fixedLayer = [...outerWallsLayerData];
        let {p, sTL, sTC, sTR, sMC, sML, sMR, sBL, sBC, sBR, cTL, cTR, cBL, cBR} = spotTiles;
        // fix patterns:
    ...
    fillEmptyTilesBetweenBordersAndSpotGround(spotLayer, additionalBordersLayer, width, height, spotTile)
    {
        let spotLayerClone = [...spotLayer];
        let mergedLayer = this.mergeLayers(spotLayerClone, [...additionalBordersLayer]);
    ...
    mergeLayers(layerA, bordersLayerClone)
    {
        let mergedLayer = Array(layerA.length).fill(0);
        for(let i = 0; i < layerA.length; i++){
            if(0 !== bordersLayerClone[i]){
                mergedLayer[i] = bordersLayerClone[i];
            }
    ...
    applySplitBordersAndCorners(applyCornersTiles, width, height, splitBordersInLayers, mainLayer, tilesShortcuts)
    {
        ...
        return bordersLayer.map(tile => tile === tilesShortcuts.p ? 0 : tile);
    }
    ...
    createRandomPathLayer(groundSpotConfig, spotTile, bordersLayer, wallsLayer, outerWallsLayer)
    {
        ...
        for(let i = 0; i < pathTileIndexes.length; i++){
            ...
            if(groundSpotConfig.applyCornersTiles){
                ...
                if(wallsLayer){
                    wallsLayer[tileIndex + groundSpotConfig.width] = 0;
                    wallsLayer[tileIndex + (groundSpotConfig.width * 2)] = 0;
                }
                if(outerWallsLayer){
                    outerWallsLayer[tileIndex + groundSpotConfig.width] = 0;
                    outerWallsLayer[tileIndex + (groundSpotConfig.width * 2)] = 0;
                    outerWallsLayer[tileIndex + (groundSpotConfig.width * 3)] = 0;
                }
            }
        }
        return {
