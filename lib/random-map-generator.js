...
class RandomMapGenerator
{
    ...
    generateLayersList()
    {
        ...
        if(this.splitBorderLayer && 0 < this.splitBorderLayer.length){
            this.staticLayers.push(this.generateLayerWithData('path-borders'+this.splitBordersLayerSuffix, this.splitBorderLayer));
        }
            ...
            for(let i = 0; i < spotsQuantity; i++){
                ...
                if(groundSpotConfig.applyCornersTiles){
                    ...
                    if(groundSpotConfig.borderInnerWalls){
                        wallsLayer = this.createSpotInnerWalls(
                            additionalBordersLayer,
                            tilesKey,
                            spotTilesShortcuts
                        );
                ...
                let pathLayer = this.createPathLayer(groundSpotConfig, additionalBordersLayer, spotTile, wallsLayer);
    ...
    {
        let innerWallsTilesShortcuts = this.mapTilesShortcuts(
            tilesKey + '-inner-walls',
            spotTilesShortcuts.p,
            this.elementsProvider.groundSpotsPropertiesMappers[tilesKey + '-inner-walls']
        );
    ...
    fillEmptyTilesBetweenBordersAndSpotGround(spotLayer, additionalBordersLayer, width, height, spotTile)
    {
        ...
        let bordersLayerClone = [...additionalBordersLayer];
        let mergedLayer = Array(spotLayerClone.length).fill(0);
        for(let i = 0; i < spotLayerClone.length; i++){
            if(0 !== bordersLayerClone[i]){
                mergedLayer[i] = bordersLayerClone[i];
            }
            if(0 === mergedLayer[i] && 0 !== spotLayerClone[i]){
                mergedLayer[i] = spotLayerClone[i];
            }
        }
            ...
            for(let x = 0; x < width; x++){
                if(0 !== row[x]){
                    if(-1 !== firstNonZeroIndex){
                        let hasSpotTile = 0 !== spotLayerClone[rowStart + firstNonZeroIndex] || 0 !== spotLayerClone[rowStart + x];
                        if(hasSpotTile){
        ...
        for(let x = 0; x < width; x++){
            ...
            for(let y = 0; y < height; y++){
                ...
                if(0 !== mergedLayer[index]){
                    if(-1 !== firstNonZeroIndex){
                        let hasSpotTile = 0 !== spotLayerClone[firstNonZeroIndex * width + x] || 0 !== spotLayerClone[y * width + x];
                        if(hasSpotTile){
    ...
    createPathLayer(groundSpotConfig, bordersLayer, spotTile, wallsLayer)
    {
        ...
        for(let i = 0; i < pathTileIndexes.length; i++){
            ...
            if(groundSpotConfig.applyCornersTiles){
                ...
                if(wallsLayer){
                    ...
                    wallsLayer[tileIndex + width] = 0;
                    wallsLayer[tileIndex + (width * 2)] = 0;
                }
            }
        }
        return {
