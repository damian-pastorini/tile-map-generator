...
class RandomMapGenerator
{
    ...
    setOptions(options)
    {
        ...
        this.pathSize = sc.get(options, 'pathSize', false);
    ...
    determineElementFreeSpaceAround(elementType, elementsFreeSpaceAround)
    {
        ...
        return Math.max(sc.get(elementsFreeSpaceAround, elementType, 0), this.minimumElementsFreeSpaceAround);
    }
    ...
    canPlaceElementCentered(x, y, width, height, placedElements)
    {
        ...
        for(let element of placedElements){
            let freeSpace = Math.max(sc.get(element, 'freeSpaceAround', 0), this.minimumElementsFreeSpaceAround);
            let e1Left = x;
            let e1Right = x + width;
            let e1Top = y;
    ...
    generateSpots()
    {
        for(let spotKey of Object.keys(this.groundSpots)){
            ...
            if('' !== spotTilesShortcuts.p && 0 !== spotTilesShortcuts.p && spotTile !== spotTilesShortcuts.p){
    ...
    mapTilesShortcuts(tilesKey, mainTile, propertiesMapper)
    {
        ...
        if(0 === mainTile && mappedData.surroundingTilesPosition['middle-center'] && 'path' === tilesKey){
            mainTile = mappedData.surroundingTilesPosition['middle-center'];
            this.pathTile = mainTile;
        }
