...
class RandomMapGenerator
{
    ...
    async fromComposite(props)
    {
        return this.fromAssociation(props);
    }
    ...
    cleanAutoGeneratedProcessMapFiles()
    {
        ...
        const filePrefix = 'optimized-';
        const fileSuffix = '-elements';
        const fileFactor = '-x';
        const fileExtensions = ['.png', '.json'];
        for (let extension of fileExtensions){
            let mapPath1 = this.mapFileFullPath.toString().replace('.json', '');
            let mapFile = this.mapFileName.toString().replace('.json', '');
            let optimizedElementsFile = mapPath1.replace(mapFile, filePrefix+mapFile+fileSuffix+extension);
            let mapPath2 = this.mapFileFullPath.toString().replace('.json', '');
            let optimizedElementsFileFactor = mapPath2.replace(
                mapFile,
                filePrefix+mapFile+fileSuffix+fileFactor+this.elementsProvider.factor+extension
            );
        ...
        for(let i = 0; i < elementData.height; i++){
            for(let j = 0; j < elementData.width; j++){
                ...
                if(elementData.data[tileIndex] !== 0){
                    ...
                    if(
                        -1 !== layer.name.indexOf('change-points')
                        && !sc.hasOwn(this.generatedChangePoints, elementData.name+'-'+elementNumber)
                    ){
                        this.generatedChangePoints[elementData.name+'-'+elementNumber] = {
                            elementData,
                            tileIndex,
                            mapIndex,
                            elementNumber,
    ...
    placeElementOnMap(elementType, elementNumber)
    {
        ...
        let position = this.findPosition(baseElementData.width, baseElementData.height);
        if (!position) {
            Logger.warning('Position not found for element "' + elementType + '".', position);
            return;
        }
    ...
    placeMainPath()
    {
        ...
        if (0 < this.generatedMainPathIndexes.length) {
            for (let mainPathPoint of this.generatedMainPathIndexes) {
                ...
                this.placeMainPathIndex(index, y, x);
            }
            return;
        }
    ...
    generateOppositeMainPath()
    {
        ...
        let topToDown = 0 === this.previousMainPath[previousPathMiddleTile].y;
