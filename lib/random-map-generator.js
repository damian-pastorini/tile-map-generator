...
class RandomMapGenerator
{
    ...
    fixEmptyTilesBetweenBordersAndSpotGround(spotLayer, additionalBordersLayer, width, height, spotTile)
    {
        ...
        for(let y = 0; y < height; y++){
            for(let x = 0; x < width - 2; x++){
                ...
                let currentIndex = rowStart + x;
                let nextIndex = rowStart + x + 1;
                let twoAheadIndex = rowStart + x + 2;
                if(0 !== mergedLayer[currentIndex] && 0 === mergedLayer[nextIndex] && 0 !== mergedLayer[twoAheadIndex]){
                    if(0 !== spotLayerClone[currentIndex] || 0 !== spotLayerClone[twoAheadIndex]){
                        resultLayer[nextIndex] = spotTile;
                    }
                }
            }
            for(let x = 0; x < width - 3; x++){
                ...
                let nextIndex = rowStart + x + 1;
                let twoAheadIndex = rowStart + x + 2;
                let threeAheadIndex = rowStart + x + 3;
                if(0 !== mergedLayer[currentIndex] && 0 === mergedLayer[nextIndex] && 0 === mergedLayer[twoAheadIndex] && 0 !== mergedLayer[threeAheadIndex]){
                    if(0 !== spotLayerClone[currentIndex] || 0 !== spotLayerClone[threeAheadIndex]){
                        resultLayer[nextIndex] = spotTile;
                        resultLayer[twoAheadIndex] = spotTile;
                    }
                }
            }
        }
    ...
    applySpotBordersAndCorners(groundSpotConfig, spotLayer, spotTilesShortcuts)
    {
        ...
        return bordersLayer.map(tile => tile === spotTilesShortcuts.p ? 0 : tile);
    }
