...
class RandomMapGenerator
{
    ...
    setOptions(options)
    {
        ...
        this.propertiesMapper.populateWithSurroundingTiles(this.surroundingTiles);
        this.corners = sc.get(options, 'corners', {});
        this.propertiesMapper.populateWithCornerTiles(this.corners);
        this.tilesShortcuts = this.mapSurroundingTilesShortcuts(
            this.pathTile,
            this.propertiesMapper.surroundingTilesPosition,
            this.propertiesMapper.cornersPosition
        );
    ...
    mapSurroundingTilesShortcuts(pathTile, surroundingTilesPosition, cornersPosition, prefix = '')
    {
        return {
            p: pathTile,
            sTL: surroundingTilesPosition[prefix+'top-left'],
            sTC: surroundingTilesPosition[prefix+'top-center'],
            sTR: surroundingTilesPosition[prefix+'top-right'],
            sML: surroundingTilesPosition[prefix+'middle-left'],
            // sMC: '121', // since 'middle-center' is directly assigned
            sMR: surroundingTilesPosition[prefix+'middle-right'],
            sBL: surroundingTilesPosition[prefix+'bottom-left'],
            sBC: surroundingTilesPosition[prefix+'bottom-center'],
            sBR: surroundingTilesPosition[prefix+'bottom-right'],
            cTL: cornersPosition[prefix+'top-left'],
            cTR: cornersPosition[prefix+'top-right'],
            cBL: cornersPosition[prefix+'bottom-left'],
            cBR: cornersPosition[prefix+'bottom-right']
        };
    }
     ...
     */
    calculateMapSizeWithFreeSpace(layerElements, elementsQuantity, freeSpaceTilesQuantity, elementsFreeSpaceAround)
    {
    ...
    determineElementAllowPathsInFreeSpace(elementType, allowPathsInFreeSpace)
    {
        if(!sc.isObject(allowPathsInFreeSpace)){
            allowPathsInFreeSpace = this.elementsAllowPathsInFreeSpace;
        }
    ...
    updateLayerData(elementData, elementNumber, baseElementData)
    {
        ...
        for(let y = 0; y < elementData.height; y++){
            for(let x = 0; x < elementData.width; x++){
                ...
                // only mark the tiles around the elements:
                // if((0 === y || (elementData.height - 1) === y) || (0 === x || (elementData.width - 1) === x)){
    ...
    markFreeSpaceAroundElementAsNotAvailable(freeSpaceAround, gridY, gridX, elementName, allowPathsInFreeSpace)
    {
        ...
        for(let i = 1; i <= freeSpaceAround; i++) {
            ...
            let savaPoints = {previousTileY, previousTileX, nextTileY, nextTileX};
            if(-1 === this.temporalBlockedPositionsToAvoidElements.indexOf(savaPoints)){
                ...
                this.temporalBlockedPositionsToAvoidElements.push({
                    previousTileY,
                    previousTileX,
                    nextTileY,
                    nextTileX,
                    elementName,
                    allowPathsInFreeSpace
                });
            }
        }
    }
    ...
    placeElementOnMap(elementType, elementNumber, position = false)
    {
        ...
        if(!position){
            position = this.findPosition(
                ...
                baseElementData.width + baseElementData.freeSpaceAround * 2,
                baseElementData.height + baseElementData.freeSpaceAround * 2
            );
        }
        if(!position){
    ...
    applyBordersAndCornersTiles(layerData, mapWidth, mapHeight, tilesShortcuts)
    {
        ...
        this.replaceSequence(layerData, [p, '0', p].join(','), [p, p, p].join(','), this.mapWidth);
        this.replaceSequence(layerData, ['0', p].join(','), [sML, p].join(','), this.mapWidth);
        this.replaceSequence(layerData, [p, '0'].join(','), [p, sMR].join(','), this.mapWidth);
        // rotate the path:
        layerData = this.rotateLayer90Degrees(layerData, mapWidth, mapHeight);
        this.replaceSequence(layerData, [p, '0', p].join(','), [p, p, p].join(','), this.mapHeight);
        this.replaceSequence(layerData, ['0', p].join(','), [sTC, p].join(','), this.mapHeight);
        this.replaceSequence(layerData, [p, '0'].join(','), [p, sBC].join(','), this.mapHeight);
        // rollback rotation:
        layerData = this.rollbackRotation90Degrees(layerData, mapHeight, mapWidth);
        // add corners:
        this.replaceSequence(layerData, ['0', sTC].join(','), [sTL, sTC].join(','), this.mapWidth);
        this.replaceSequence(layerData, [sTC, '0'].join(','), [sTC, sTR].join(','), this.mapWidth);
        this.replaceSequence(layerData, ['0', sBC].join(','), [sBL, sBC].join(','), this.mapWidth);
        this.replaceSequence(layerData, [sBC, '0'].join(','), [sBC, sBR].join(','), this.mapWidth);
        this.replaceSequence(layerData, [sBC, sML].join(','), [sBC, cBL].join(','), this.mapWidth);
        this.replaceSequence(layerData, [sTC, sML].join(','), [sTC, cTL].join(','), this.mapWidth);
        this.replaceSequence(layerData, [sMR, sBC].join(','), [cBR, sBC].join(','), this.mapWidth);
        this.replaceSequence(layerData, [sMR, sTC].join(','), [cTR, sTC].join(','), this.mapWidth);
        // rotate to add upper corners:
        layerData = this.rotateLayer90Degrees(layerData, mapWidth, mapHeight);
        this.replaceSequence(layerData, [sML, '0'].join(','), [sML, sBL].join(','), this.mapHeight);
        this.replaceSequence(layerData, ['0', sML].join(','), [sTL, sML].join(','), this.mapHeight);
        this.replaceSequence(layerData, [cBR, '0'].join(','), [cBR, sBL].join(','), this.mapHeight);
        this.replaceSequence(layerData, [sMR, '0'].join(','), [sMR, sBR].join(','), this.mapHeight);
        this.replaceSequence(layerData, [sMR, p].join(','), [cTR, p].join(','), this.mapHeight);
        this.replaceSequence(layerData, [p, sMR].join(','), [p, cBR].join(','), this.mapHeight);
        this.replaceSequence(layerData, [p, sML].join(','), [p, cBL].join(','), this.mapHeight);
        this.replaceSequence(layerData, [sML, p].join(','), [cTL, p].join(','), this.mapHeight);
        this.replaceSequence(layerData, ['0', sMR].join(','), [sTR, sMR].join(','), this.mapHeight);
        this.replaceSequence(layerData, [cBL, '0'].join(','), [cBL, sBL].join(','), this.mapHeight);
        this.replaceSequence(layerData, ['0', cBL].join(','), [sTR, cBL].join(','), this.mapHeight);
        this.replaceSequence(layerData, ['0', cTL].join(','), [sTL, cTL].join(','), this.mapHeight);
        layerData = this.rollbackRotation90Degrees(layerData, mapHeight, mapWidth);
        // restore rotation and fix round corners:
        this.replaceSequence(layerData, [sMR, sBL].join(','), [cBR, sBL].join(','), this.mapWidth);
        this.replaceSequence(layerData, [sTL, sML].join(','), [sTL, cTL].join(','), this.mapWidth);
        this.replaceSequence(layerData, [sMR, sBR].join(','), [cBR, sBR].join(','), this.mapWidth);
        this.replaceSequence(layerData, [sBL, sML].join(','), [sBL, cBL].join(','), this.mapWidth);
        this.replaceSequence(layerData, [sMR, sTR].join(','), [cTR, sTR].join(','), this.mapWidth);
        this.replaceSequence(layerData, [cTR, '0'].join(','), [cTR, sTR].join(','), this.mapWidth);
        return layerData;
    }
    ...
    applyRotationToCompletePathGrid()
    {
        ...
        while (true) {
            this.replaceSequence(this.pathLayerData, doubleSpace, doubleReplace, this.mapWidth);
            let applyHorizontalChanges = this.replaceSequence(
                this.pathLayerData,
                singleSpace,
                singleReplace,
                this.mapWidth
            ...
            this.replaceSequence(this.pathLayerData, doubleSpace, doubleReplace, this.mapHeight);
            let applyVerticalChanges = this.replaceSequence(
                this.pathLayerData,
                singleSpace,
                singleReplace,
                this.mapHeight
    ...
    generateSpots()
    {
        for(let spotKey of Object.keys(this.groundSpots)){
            ...
            let groundSpotsPropertiesMapper = this.elementsProvider.groundSpotsPropertiesMappers[tilesKey];
            if(groundSpotsPropertiesMapper){
                if(0 === Object.keys(groundSpotsPropertiesMapper.surroundingTilesPosition).length){
                    groundSpotsPropertiesMapper.populateWithSurroundingTiles(groundSpotsPropertiesMapper.surroundingTiles);
                }
                if(0 === Object.keys(groundSpotsPropertiesMapper.cornersPosition).length){
                    groundSpotsPropertiesMapper.populateWithCornerTiles(groundSpotsPropertiesMapper.corners);
                }
            }
            ...
            if(groundSpotConfig.applyCornersTiles){
                // search for corner tiles and if exists apply those as part of the spot area
                spotTilesShortcuts = this.mapSurroundingTilesShortcuts(
                    spotTile,
                    groundSpotsPropertiesMapper.surroundingTilesPosition,
                    groundSpotsPropertiesMapper.cornersPosition,
                    tilesKey+'-'
                );
            }
            let spotLayers = {};
