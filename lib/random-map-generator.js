...
class RandomMapGenerator
{
    ...
    populateCollisionsMapBorder()
    {
        ...
        for(let x = 0; x < this.mapWidth; x++){
            // top border:
            this.borderLayer[x] = borderTile;
            // bottom border:
            this.borderLayer[(this.mapHeight - 1) * this.mapWidth + x] = borderTile;
        }
        for(let y = 0; y < this.mapHeight; y++){
            // left border:
            this.borderLayer[y * this.mapWidth] = borderTile;
            // right border:
            this.borderLayer[y * this.mapWidth + (this.mapWidth - 1)] = borderTile;
        }
        if(this.validateBorderCorners()){
            this.borderLayer[0] = this.borderCornersTiles['border-top-left'];
            this.borderLayer[this.mapWidth - 1] = this.borderCornersTiles['border-top-right'];
            this.borderLayer[(this.mapHeight - 1) * this.mapWidth] = this.borderCornersTiles['border-bottom-left'];
            this.borderLayer[(this.mapHeight - 1) * this.mapWidth + this.mapWidth - 1] = this.borderCornersTiles['border-bottom-right'];
        }
    ...
    {
        if(0 === Object.keys(this.borderCornersTiles).length){
            return false;
        }
        return this.borderCornersTiles['border-top-left']
            && this.borderCornersTiles['border-top-right']
            && this.borderCornersTiles['border-bottom-left']
            && this.borderCornersTiles['border-bottom-right'];
    }
