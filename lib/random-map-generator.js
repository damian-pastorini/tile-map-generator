    ...
    cleanAutoGeneratedProcessMapFiles()
    {
        ...
        for (let extension of fileExtensions){
            ...
            let optimizedElementsFileFactor = mapPath2.replace(
                this.mapName,
                filePrefix+this.mapName+fileSuffix+fileFactor+this.elementsProvider.factor+extension
            );
            ...
            for(let j = 0; j < elementData.width; j++){
                ...
                if(elementData.data[tileIndex] !== 0){
                    ...
                    if(
                        ...
                        && !sc.hasOwn(this.generatedChangePoints, mapPrefix+elementData.name+'-'+elementNumber)
                    ){
                        this.generatedChangePoints[mapPrefix+elementData.name+'-'+elementNumber] = {
                            elementData,
                            tileIndex,
                            mapIndex,
                            elementNumber,
