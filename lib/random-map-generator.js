...
class RandomMapGenerator
{
    ...
    async generate()
    {
        ...
        let gridResult = this.mapGridBuilder.generateEmptyMap(this.mapSize, this.groundTile, this.calculateMapSizeWithFreeSpace.bind(this));
    ...
    placeMainPathIndex(index, y, x)
    {
        try {
            ...
            // mark as occupied:
            this.mapGridBuilder.markMapGridPosition(this.mapGrid, y, x, false);
        } catch (error) {
    ...
    async connectPaths()
    {
        ...
        let gridResult = this.mapGridBuilder.createPathfindingGrid(
            ...
            this.mapGrid,
            this.tileIndexByRowAndColumn.bind(this)
        );
        let grid = gridResult.grid;
        this.debugLayerData = gridResult.debugLayerData;
        let allPathsPoints = [];
        let pathLayers = this.additionalLayers.filter(layer => 'path' === layer.name);
        // ensure the main path is walkable:
        ...
        for(let layer of pathLayers){
            ...
            for(let i = 0; i < pathTilePositions.length; i++){
                ...
                // ensure the path tile position is walkable:
        ...
        this.splitBorderLayer = this.applyBorderEndTiles(
            this.splitBorderLayer,
            this.fetchPathBorderEndTiles(),
            this.splitBorderLayer,
            this.tilesShortcuts,
            this.mapWidth,
            this.mapHeight
        ...
        if(outerWallsResult?.outerWallsLayer && outerWallsResult?.bordersLayer){
            ...
            this.pathInnerWallsLayer = this.fixInnerWallsPatterns(
                this.pathInnerWallsLayer,
                this.fetchTilesShortcutsBySuffix(this.pathsInnerWallsTilesKey, this.tilesShortcuts, '-inner-walls'),
                this.mapWidth,
                this.mapHeight
            );
        }
        ...
        this.markPathTilesAsUnavailable();
        this.pathLayerData = this.fillPathSingleTiles(this.pathLayerData, this.pathTile, this.mapWidth, this.mapHeight);
    ...
    fetchPathBorderEndTiles()
    {
        let pathBorderEndTiles = [];
        for(let tileIndex = 0; tileIndex < this.splitBorderLayer.length; tileIndex++){
            let tileConnections = this.connectedTiles(tileIndex, this.splitBorderLayer, this.mapWidth);
    ...
    applyCleanPathBorderTiles()
    {
        ...
        for(let y = 0; y < this.mapHeight; y++){
            for(let x = 0; x < this.mapWidth; x++){
                let index = y * this.mapWidth + x;
                if(this.pathTile !== this.splitBorderLayer[index]
    ...
    async generateSpots()
    {
        for(let spotKey of Object.keys(this.groundSpots)){
            ...
            this.populatePropertiesMapper(this.groundSpotsPropertiesMappers[tilesKey]);
    ...
    fetchTilesShortcutsBySuffix(tilesKey, spotTiles, suffix)
    {
        ...
        return this.mapTilesShortcuts(
            wangsetKey,
            spotTiles.p,
            this.groundSpotsPropertiesMappers[wangsetKey]
        );
    }
    ...
    {
        // @TODO - BETA - Make the 5 configurable, this is used as minimum distance from the borders.
    ...
    mapTilesShortcuts(tilesKey, mainTile, propertiesMapper)
    {
        ...
        // as default, get the tiles from the property mapper:
        let mappedData = {
            surroundingTilesPosition: propertiesMapper?.surroundingTilesPosition,
            cornersPosition: propertiesMapper?.cornersPosition
        }
        // if property mapper does not have the tile info, then try to fetch the data from the wangsets:
        if(
            !propertiesMapper
    ...
    {
        let optimizedMapTileset = this.optimizedMapFirstTileset;
