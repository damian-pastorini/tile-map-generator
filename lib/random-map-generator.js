...
class RandomMapGenerator
{
    ...
    generateLayersList()
    {
        ...
        this.preMergeLayers = sc.deepJsonClone(layers);
    ...
    {
        if(-1 === this.additionalLayers.findIndex(layer => layer.name === elementData.name)){
        ...
        let layerIndex = this.additionalLayers.findIndex(layer => layer.name === targetLayerName);
        if(-1 === layerIndex){
            this.additionalLayers.push(
                this.generateLayerWithData(targetLayerName, Array(this.mapWidth * this.mapHeight).fill(0))
            );
            layerIndex = this.additionalLayers.length - 1;
        }
    ...
    placeElementOnMap(elementType, elementNumber, position = false)
    {
        ...
        for(let elementLayer of elementLayersDataArray){
            ...
            this.updateLayerData(elementLayer, elementNumber, baseElementData, elementType);
        }
    }
