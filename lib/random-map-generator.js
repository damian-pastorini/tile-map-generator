...
class RandomMapGenerator
{
    ...
    constructor(props)
    {
        ...
        this.spotsTerrains = {};
        this.tilesShortcuts = {};
        this.applyMapBorderInnerWalls = false;
        this.mapBorderWallsLayerSuffix = '-collisions';
        this.borderWallsLayer = false;
    ...
    assignBaseOptions(options)
    {
        ...
        if(this.isGroundTilesSelection()){
            this.groundTile = this.groundTiles[Math.floor(Math.random() * this.groundTiles.length)];
        }
    }

    isGroundTilesSelection()
    {
        if(1 < this.groundTiles.length){
            return true;
        }
        return 0 === this.groundTile && 0 < this.groundTiles.length;
    }
    ...
    assignPathAndBorderOptions(options)
    {
        ...
        this.borderInnerCornersTiles = sc.get(options, 'borderInnerCornersTiles', {});
        ...
        this.applyMapBorderInnerWalls = sc.get(options, 'applyMapBorderInnerWalls', false);
        this.mapBorderWallsLayerSuffix = sc.get(options, 'mapBorderWallsLayerSuffix', '-collisions');
        this.borderWallsLayer = false;
    ...
    async generate()
    {
        ...
        this.mapBorderGenerator.createEntryPosition();
