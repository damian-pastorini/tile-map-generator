    ...
    updateLayerData(elementData, elementNumber)
    {
        ...
        for(let i = 0; i < elementData.height; i++){
            for(let j = 0; j < elementData.width; j++){
                ...
                if (-1 === elementData.name.indexOf('stairs')) {
                    elementKey = elementKey+'-'+elementNameClean+'-n'+elementNumber;
                }
                if (-1 !== elementData.name.indexOf('stairs')) {
                    let nextUpperFloor = currentFloorNumber + 1;
                    let nextDownFloor = currentFloorNumber - 1;
                    let upperFloorString = '' === currentFloorKey || 0 === nextUpperFloor
                        ? ''
                        : '-'+currentFloorKey+'Floor-n'+ nextUpperFloor;
                    let downFloorString = '' === currentFloorKey || 0 === nextDownFloor
                        ? ''
                        : '-'+currentFloorKey+'Floor-n'+ nextDownFloor;
                    elementKey = elementKey+elementNameClean
