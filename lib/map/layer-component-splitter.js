/**
 *
 * Reldens - Tile Map Generator - LayerComponentSplitter
 *
 * Splits Tiled tilelayer entries into per-connected-component layers so consumers (e.g. the
 * reldens map-elements records emitter) can produce one element record per placed instance
 * even when the generator stamped multiple placements of the same element into a single
 * shared layer (e.g., 'tree-001-base' with 5 placements). Element layers are grouped by
 * source instanceId; connected components are found in the UNION of all layers for that
 * instance (so base/collisions/etc. components line up spatially); each component yields one
 * renamed layer per source layer with a globally unique counter per elementKey base, so the
 * records loader produces distinct instanceIds across the whole map.
 *
 */
...
class LayerComponentSplitter
{
    ...
    splitGroup(result, group, mapWidth, mapHeight, baseCounters)
    {
        ...
        let unionData = this.buildUnionData(group.layers, totalCells, mapWidth, mapHeight);
        let components = this.findConnectedComponents(unionData, mapWidth, mapHeight);
        if(2 > components.length){
            this.appendAllLayers(result, group.layers);
            return;
        }
        this.appendSplitInstances(result, group, components, totalCells, baseCounters, mapWidth, mapHeight);
    }
    ...
    appendSplitInstances(result, group, components, totalCells, baseCounters, mapWidth, mapHeight)
    {
        ...
        for(let i = 0; i < components.length; i++){
            ...
            this.appendInstanceLayers(result, group, components[i], totalCells, globalIndex, mapWidth, mapHeight);
        }
    }

    appendInstanceLayers(result, group, component, totalCells, globalIndex, mapWidth, mapHeight)
    {
        ...
        for(let li = 0; li < group.layers.length; li++){
            ...
            perInstance.data = this.buildLayerDataFromIndices(layer.data, cellIndices, totalCells, mapWidth, mapHeight);
            result.push(perInstance);
        }
    }

    buildUnionData(layers, totalCells, mapWidth, mapHeight)
    {
    ...
    {
        let rowAndColumn = this.layerDataFactory.rowAndColumnByTileIndex(cellIndex, mapWidth);
        let col = rowAndColumn.column;
    ...
    buildLayerDataFromIndices(sourceData, cellIndices, totalCells, mapWidth, mapHeight)
    {
        let data = this.layerDataFactory.createEmptyLayerData(mapWidth, mapHeight);
