...
class LayerDataFactory
{
    ...
    buildDataFromTiles(tiles, mapWidth, mapHeight)
    {
        let data = this.createEmptyLayerData(mapWidth, mapHeight);
        this.writeTilesToData(data, tiles, mapWidth, tile => tile.gid);
        return data;
    }
    ...
    appendLayerRows(data, width, rowsCount, fillValue)
    {
        if(!data || 0 === data.length){
            return data;
        }
        for(let index = 0; index < width * rowsCount; index++){
            data.push(fillValue);
        }
        return data;
    }
