...
class ElementsToLayersBuilder
{
    ...
    apply(mapJson, mapElements)
    {
        ...
        let layersByType = this.groupLayersByType(elementLayers);
        let mergedElementLayers = [];
        for(let layerType of Object.keys(layersByType)){
            mergedElementLayers.push(...this.layerDataFactory.mergeLayersByNameSubstring(layersByType[layerType], layerType));
        }
        mapJson.layers = this.mergeStaticAndElementLayers(mapJson.layers, mergedElementLayers);
        return mapJson;
    }
     ...
     */
    groupLayersByType(elementLayers)
    {
        let layersLoader = new ElementsFromLayersLoader();
        let layersByType = {};
        for(let layer of elementLayers){
            let parsed = layersLoader.elementLayerName.parse(layer.name);
            let layerType = parsed ? parsed.layerType : 'base';
            if(!layersByType[layerType]){
                layersByType[layerType] = [];
            }
