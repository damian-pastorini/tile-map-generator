...
class ElementsToLayersBuilder
{
    ...
    spotMergeKey(layer)
    {
        ...
        if(null !== this.elementsLoader.elementLayerName.parse(layer.name)){
            return null;
        }
        return this.spotGroupKey(layer.name);
    }
    ...
    buildElementLayer(elementLayer, mapWidth, mapHeight)
    {
        ...
        return this.layerDataFactory.buildTileLayer(elementLayer.name, data, mapWidth, mapHeight);
    }
