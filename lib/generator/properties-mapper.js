...
class PropertiesMapper
{
    ...
    map(surroundingTiles = {}, corners = {})
    {
        if(0 === Object.keys(surroundingTiles).length && this.surroundingTiles){
            surroundingTiles = this.surroundingTiles;
        }
        if(0 === Object.keys(corners).length && this.corners){
            corners = this.corners;
        }
        if(0 === Object.keys(this.surroundingTilesPosition).length){
            this.populateWithSurroundingTiles(surroundingTiles);
        }
        if(0 === Object.keys(this.cornersPosition).length){
            this.populateWithCornerTiles(corners);
        }
        return this;
    }
