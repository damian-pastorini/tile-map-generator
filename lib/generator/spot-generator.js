...
class SpotGenerator
{
    ...
    constructor(generator)
    {
        ...
        this.populatePropertiesMapper = generator.populatePropertiesMapper.bind(generator);
        ...
        this.randomCentralIndex = generator.randomCentralIndex.bind(generator);
        this.get4Neighbors = generator.get4Neighbors.bind(generator);
    }
    ...
    {
        let invisibleSpotsKeys = Object.keys(generatedSpots).filter(
        ...
        for(let spotKey of invisibleSpotsKeys){
            ...
            for(let spotLayerKey of Object.keys(groundSpotConfig.spotLayers)){
                ...
                let randomX = Math.floor(Math.random() * (mapWidth - groundSpotConfig.width));
                let randomY = Math.floor(Math.random() * (mapHeight - groundSpotConfig.height));
                for(let y = 0; y < groundSpotConfig.height; y++){
                    for(let x = 0; x < groundSpotConfig.width; x++){
                        ...
                        let gridX = randomX + x;
                        let gridY = randomY + y;
                        let mapIndex = gridY * mapWidth + gridX;
    ...
    saveLayerElements(
        ...
        return {layerElements, elementsQuantity, elementsFreeSpaceAround, elementsAllowPathsInFreeSpace, mapCenteredElements};
    }
