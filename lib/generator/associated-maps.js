...
class AssociatedMaps
{
    ...
    async generate(mapJson, mapName, rootFolder, associatedMapsConfig, mainMapGenerator)
    {
        ...
        for(let changePointKey of generatedChangePoints){
            ...
            if(!changePointLayer){
                Logger.warning('Change point layer not found, associated map skipped.', targetLayerName, mapName);
                continue;
            }
            ...
            let subMapName = this.generateSubMapName(mappedProperties, mapName, generatedChangePoint, changePointKey);
    ...
    generateSubMapName(mappedProperties, mapName, changePointData, changePointKey)
    {
        let subMapNameProperty = sc.get(mappedProperties, 'subMapName', '');
        if('' === subMapNameProperty){
            return changePointKey;
        }
        return mapName + '-' + subMapNameProperty + '-n' + sc.get(changePointData, 'elementNumber', 'n');
    }
