...
class MapBorderGenerator
{
    ...
    createEntryPosition()
    {
        ...
        this.generator.additionalLayers.unshift(generatedLayer);
    }
    ...
    stampEntryPositionEnds(direction, x, y)
    {
        ...
        let leftColumn = x - 1;
        let rightColumn = x + this.generator.entryPositionSize;
        let openingLeftEnd = this.fetchOpeningEndTile(isTopBorder, 'left', this.isCornerColumn(leftColumn));
        let openingRightEnd = this.fetchOpeningEndTile(isTopBorder, 'right', this.isCornerColumn(rightColumn));
        this.stampBorderEnd(y, leftColumn, openingLeftEnd);
        this.stampBorderEnd(y, rightColumn, openingRightEnd);
        return true;
    }

    isCornerColumn(column)
    {
        if(0 === column){
            return true;
        }
        return column === this.generator.mapWidth - 1;
    }

    fetchOpeningEndTile(isTopBorder, side, isCornerColumn)
    {
        let borderFamilyKey = isTopBorder ? 'top-' : 'bottom-';
        let innerFamilyKey = isTopBorder ? 'bottom-' : 'top-';
        let oppositeSide = 'left' === side ? 'right' : 'left';
        let familyKey = isCornerColumn ? borderFamilyKey : innerFamilyKey;
        let innerCornerTile = sc.get(this.generator.borderInnerCornersTiles, familyKey+oppositeSide, 0);
        if(innerCornerTile){
