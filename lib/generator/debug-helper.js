...
class DebugHelper
{
    ...
    async debugAdjacentSpots(firstElementPosition, firstElementSize, elementFreeSpace, mapWidth, mapHeight)
    {
        ...
        for(let i = 0; i < placementOffsets.length; i++){
            ...
            let offsetX = offset.x * (width + freeSpaceAround * this.generator.requiredForBothSidesDuplicator);
            let offsetY = offset.y * (height + freeSpaceAround * this.generator.requiredForBothSidesDuplicator);
