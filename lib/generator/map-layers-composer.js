...
class MapLayersComposer
{
    ...
    generateLayersList()
    {
        this.generator.staticLayers = this.generator.spotGenerator.generateInvisibleSpots(
    ...
    tryMergeCandidate(layers, j, used, matchKey, data, name)
    {
        ...
        if(!collision){
            data = this.fillEmptyTilesFromCandidate(data, candidate.data);
            name += "-" + candidate.name;
            used.add(j);
        }
        return {data, name};
    ...
    fillEmptyTilesFromCandidate(data, candidateData)
    {
        let filledData = data;
        for(let i = 0; i < filledData.length; i++){
            if(0 === filledData[i]){
                filledData[i] = candidateData[i];
            }
        }
        return filledData;
    }
