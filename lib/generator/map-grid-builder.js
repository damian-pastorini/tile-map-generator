...
class MapGridBuilder
{
    ...
    calculateMapSizeWithFreeSpace()
    {
        ...
        for(let elementType of Object.keys(this.elementsQuantity)){
            ...
            let widthPlusFreeTiles = element.width + (freeTilesUpDownLeftRight + freeSpaceUpDownLeftRight) * 2;
            let heightPlusFreeTiles = element.height + (freeTilesUpDownLeftRight + freeSpaceUpDownLeftRight) * 2;
        ...
        for(let spotKey of Object.keys(spotsForSizing)){
            ...
            if(spotConfig.isElement){
                let spotFreeSpace = Number(sc.get(spotConfig, 'freeSpaceAround', 1));
                spotWidth += spotFreeSpace * 2;
                spotHeight += spotFreeSpace * 2;
            }
