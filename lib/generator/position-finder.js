...
class PositionFinder
{
    ...
    findRandomPositionOnAnywhere(width, height, mapWidth, mapHeight, mapGrid)
    {
        ...
        while(tries < maxTries){
            let x = Math.floor(Math.random() * (mapWidth - width));
            let y = Math.floor(Math.random() * (mapHeight - height));
            if(this.canPlaceElement(x, y, width, height, mapWidth, mapHeight, mapGrid)){
                return {x, y};
            }
            tries++;
        }
