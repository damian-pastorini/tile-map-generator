...
class CenteredElementsPlacer
{
    ...
    async placeFirstCenteredElement(orderedElements, mapCenterX, mapCenterY, placedElements)
    {
        ...
        let expandedWidth = firstElement.width + freeSpaceAround * this.generator.requiredForBothSidesDuplicator;
        let expandedHeight = firstElement.height + freeSpaceAround * this.generator.requiredForBothSidesDuplicator;
        let validator = this.elementsPlacer.feasibility.buildValidator(
            firstElementKey,
            expandedWidth,
            expandedHeight,
            -freeSpaceAround
        );
        let canPlaceFirst = false !== validator && this.canPlaceElementCentered(
            posX,
            posY,
            firstElement.width,
            firstElement.height,
        ...
        if(!canPlaceFirst){
            ...
            if(this.resolveRejectedCenteredPlacement(firstElementKey, 0, placedElements)){
    ...
    placeCenteredElementInstances(elementKey, quantity, placement)
    {
        for(let i = 0; i < quantity; i++){
            ...
            if(!placed && !this.resolveRejectedCenteredPlacement(elementKey, i, placement.placedElements)){
    ...
    tryPlaceCenteredElement(elementKey, instanceIndex, element, elementFreeSpace, placement)
    {
        ...
        let expandedWidth = element.width + elementFreeSpace * this.generator.requiredForBothSidesDuplicator;
        let expandedHeight = element.height + elementFreeSpace * this.generator.requiredForBothSidesDuplicator;
        let validator = this.elementsPlacer.feasibility.buildValidator(
            elementKey,
            expandedWidth,
            expandedHeight,
        ...
        for(let attempts = 0; attempts < placementOffsets.length * 4 && !placed; attempts++){
            ...
            let offsetX = offset.x * expandedWidth * multiplyFactor;
            let offsetY = offset.y * expandedHeight * multiplyFactor;
            ...
            if(this.canPlaceElementCentered(posX, posY, element.width, element.height, placedElements, validator)){
    ...
    canPlaceElementCentered(x, y, width, height, placedElements, validator = null)
    {
        ...
        if(!this.generator.geometryCalculator.isFootprintWalkable(this.generator.mapGrid, x, y, width, height)){
            return false;
        }
        return !(validator && !validator(x, y));
    }

    resolveRejectedCenteredPlacement(elementKey, instanceIndex, placedElements)
    {
        if(!this.generator.placementRejectResolver.resolve(elementKey, instanceIndex)){
            return false;
        }
        let journal = this.generator.placedElementsJournal;
        let entry = journal[journal.length - 1];
        placedElements.push({
            type: entry.elementType,
            position: {x: entry.position.x, y: entry.position.y},
            width: entry.width,
            height: entry.height,
            freeSpaceAround: entry.freeSpaceAround
        });
        return true;
    }
