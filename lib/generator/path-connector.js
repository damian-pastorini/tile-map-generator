...
class PathConnector
{
    ...
    constructor(generator)
    {
        ...
        this.wallsGenerator = this.generator.wallsGenerator;
        this.spotGenerator = this.generator.spotGenerator;
    }
    ...
    async connectPaths(
        ...
        splitBorderLayer = this.spotGenerator.applySplitBordersAndCorners(
        ...
        splitBorderLayer = this.spotGenerator.applyBorderEndTiles(
            splitBorderLayer,
        ...
        if(outerWallsResult?.outerWallsLayer && outerWallsResult?.bordersLayer){
            ...
            pathInnerWallsLayer = this.wallsGenerator.fixInnerWallsPatterns(
                pathInnerWallsLayer,
                this.tileShortcutsMapper.mapTilesShortcuts(this.pathsInnerWallsTilesKey, tilesShortcuts.p, null, '-inner-walls'),
                mapWidth,
                mapHeight
            );
        }
    ...
    placeMainPath(
        ...
        if(0 < generatedMainPathIndexes.length){
            ...
            if(!generatedReturnPoints['default-main-path']){
                ...
                let returnPointIndex = this.tilePositionCalculator.provideReturnIndexByPosition(returnPointX, returnPointY);
    ...
    fillPathSingleTiles(pathLayerData, pathTile, mapWidth, mapHeight)
    {
        ...
        this.bordersPatterns.replaceSequences(pathLayerData, [[singleEmpty, singleFix], [doubleEmpty, doubleFix]], mapWidth);
        pathLayerData = this.bordersPatterns.rotateLayer90Degrees(pathLayerData, mapWidth, mapHeight);
        this.bordersPatterns.replaceSequences(pathLayerData, [[singleEmpty, singleFix], [doubleEmpty, doubleFix]], mapHeight);
        pathLayerData = this.bordersPatterns.rollbackRotation90Degrees(pathLayerData, mapHeight, mapWidth);
        return pathLayerData;
    }
    ...
    applyRotationToCompletePathGrid(pathTile, layerData, mapWidth, mapHeight)
    {
        ...
        while(true){
            this.bordersPatterns.replaceSequence(layerData, doubleSpace, doubleReplace, mapWidth);
            let applyHorizontalChanges = this.bordersPatterns.replaceSequence(layerData, singleSpace, singleReplace, mapWidth);
            layerData = this.bordersPatterns.rotateLayer90Degrees(layerData, mapWidth, mapHeight);
            this.bordersPatterns.replaceSequence(layerData, doubleSpace, doubleReplace, mapHeight);
            let applyVerticalChanges = this.bordersPatterns.replaceSequence(layerData, singleSpace, singleReplace, mapHeight);
